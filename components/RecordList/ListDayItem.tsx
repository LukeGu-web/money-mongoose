  ...
  return (
    <View className='p-3'>
      <View className='flex-row items-center justify-between'>
        <Text className='pb-1 text-lg font-bold'>{formattedDate}</Text>
        <View className='flex-row gap-2'>
          {item.sum_of_income > 0 && (
            <Text className='pb-1 text-lg font-bold color-green-700'>
              Income: {item.sum_of_income}
            </Text>
          )}
          {item.sum_of_expense < 0 && (
            <Text className='pb-1 text-lg font-bold color-red-700'>
